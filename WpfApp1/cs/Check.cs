...
namespace OPL_WpfApp
{
    public partial class MainWindow : Window
    {
        public void Checkopen(string m)
        {
            ...
            {
                Logs.Out_Logs("[提示]程序启动完毕，请耐心等待隧道连接"); //启动完毕
                fstert.Fill = Brushes.Green;
            }
            if (m.Contains("autorunApp end"))
            {
                Logs.Out_Logs("[提示]程序离线，请检查你的网络设置或查看网络连接是否正常");
            ...
            if (m.Contains("LISTEN ON PORT")) //连接成功or断开
            {
                ...
                if (match.Success)
                {
                    ...
                    if (m.Contains("START"))
                    {
                        Logs.Out_Logs("[提示]隧道本地端口为 " + portInfo + " 连接成功");
                        state[portInfo] = 2;
                        Relist();
                    ...
                    if (m.Contains("END"))
                    {
                        Logs.Out_Logs("[错误]隧道本地端口为 " + portInfo + " 断开连接");
                        state[portInfo] = 1;
                        Relist();
                    }
            ...
            {
                Logs.Out_Logs("[错误]请检查是否连接网络，或是程序是否拥有网络访问权限！");
            ...
            if (m.Contains("Only one usage of each socket address"))
            {
                ...
                if (match.Success)
                {
                    ...
                    Logs.Out_Logs($"[错误]: 本地端口{protocol}:{port}被占用，请更换相关本地端口");
            ...
            {
                Logs.Out_Logs("[错误]请检查DNS是否正确，是否连接网络，或是程序是否拥有网络访问权限！");
            }
            if (m.Contains("it will auto reconnect when peer node online"))//对方不在线
            {
                ...
                if (match.Success)
                {
                    ...
                    Logs.Out_Logs("[错误]" + id + "不在线！请查询对方UID是否输入错误，询问对方程序是否处于启动状态，当对方在线时会自动进行连接");
            ...
            if (m.Contains("peer offline"))//对方不在线
            {
                ...
                Logs.Out_Logs("[错误]你连接的人不在线！请查询对方UID是否输入错误，询问对方程序是否处于启动状态，当对方在线时会自动进行连接");
            ...
            if(m.Contains("NAT type"))
            {
                ...
                if (match.Success)
                {
                    ...
                    //Logs.Out_Logs("[提示]你的NAT类型为"+type);
                    if (type == "2")
                        Logs.Out_Logs("[提示]你的NAT类型为对称形 Symmetric NAT，连接可能受阻，或连接时间较长");
                }
            }
        ...
        public async Task GetsayText(bool oo = true)
        {
            ...
            try
            {
                if(oo)Logs.Out_Logs("[提示]获取一言 -UAPI公益API提供数据支持");
            ...
            {
                daysay.Text = "获取失败";
                Logs.Out_Logs("[错误]获取每日一句失败：" + ex.Message);
            }
        }
    }
}
